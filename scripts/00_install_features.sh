...
# /workspace/isia_wiki/features/, which `splent product:resolve` fills
...
# the builder stage of docker/Dockerfile.isia_wiki.prod.
#
...
features=$(python3 -c '
import os, tomllib
with open("/workspace/isia_wiki/pyproject.toml", "rb") as f:
    data = tomllib.load(f)
...
for feature in $features; do
    ...
    if [ -n "$declared_version" ]; then
        ...
        found_path=""
        for org_dir in /workspace/isia_wiki/features/*; do
            [ -d "$org_dir" ] || continue
